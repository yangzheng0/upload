...
layui.define("layer",
    function(e) {
        ...
        var progressListener = function(fun) {
            progressListener.onprogress = fun; //绑定监听
            //使用闭包实现监听绑
            return function() {
                //通过$.ajaxSettings.xhr();获得XMLHttpRequest对象
                var xhr = i.ajaxSettings.xhr();
                //判断监听函数是否为函数
                if (typeof progressListener.onprogress !== 'function')
                    return xhr;
                //如果有监听函数并且xhr对象支持绑定时就把监听函数绑定上去
                if (progressListener.onprogress && xhr.upload) {
                    xhr.upload.onprogress = progressListener.onprogress;
                }
                return xhr;
            }
        };
                            ...
                            function(e, a) {
                                ...
                                r.append(l.field, a),
                                    ...
                                    i.ajax({
                                        ...
                                        processData: !1,
                                        dataType: "json",
                                        xhr: progressListener(function(e){//此处为新添加功能
